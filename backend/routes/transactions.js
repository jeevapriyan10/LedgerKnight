...
// Create new transaction (associates only)
router.post('/', authenticateToken, requireRole('associate'), validateTransactionCreation, async (req, res) => {
    try {
        const { receiver, amountEther, purpose, comment, deadline, priority, originalTxId } = req.body || {};
        ...
        // If this is a resubmission, mark the original transaction as modified
        if (originalTxId) {
            const originalTx = await Transaction.findById(originalTxId);
            if (originalTx && originalTx.creatorId === userId) {
                originalTx.status = 4; // 4 = modified (superseded)
                await originalTx.save();
            }
        }

        // Soft balance check (warning only, doesn't block creation)
        try {
            const balEth = await eoaService.getInstitutionBalance(institutionId);
            if (parseFloat(balEth) < parseFloat(amountEther)) {
                console.log('Warning: Insufficient balance for transaction');
            }
        } catch (err) {
            console.log('Balance check failed (non-blocking):', err.message);
        }
...
// Review transaction (auditor only)
router.post('/:txId/review', authenticateToken, requireRole('auditor'), async (req, res) => {
    try {
        ...
        // Verify transaction belongs to this institution
        ...
        if (decision === 'APPROVED') {
            ...
            if (!auditorPassword) {
                return res.status(400).json({ error: 'Auditor password required for approval' });
            }
            ...
            if (parseFloat(balEth) < parseFloat(tx.amount)) {
                return res.status(400).json({ error: 'Insufficient EOA balance. Deposit funds and retry.' });
            }
            ...
            return res.status(400).json({ error: 'Invalid decision. Must be APPROVED, DECLINED, or REVIEW' });
        }
    } catch (error) {
        console.error('Review transaction error:', error);
        res.status(500).json({ error: 'Failed to review transaction: ' + error.message });
    }
});

// Get all transactions for institution
